...
Federated Learning for 30-Day Readmission PredictionThis repository contains the code and experiments for a research project on trustworthy federated learning for predicting 30-day hospital readmissions. The project uses synthetic data generated by Synthea™ and the Flower federated learning framework to simulate a multi-institutional study with a focus on privacy, fairness, and robustness.🚀 Project GoalThe primary objective is to develop and evaluate a federated learning system that can train a predictive model across three simulated hospitals without sharing raw patient data. The project explores advanced federated algorithms (FedProx) and privacy-enhancing technologies (Differential Privacy) to build a model that is not only accurate but also trustworthy and compliant with regulations like GDPR.📂 Project StructureThe repository is organized as follows:PHD_FEDERATED_LEARNING_PROJECT/
│
